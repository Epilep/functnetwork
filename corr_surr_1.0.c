...
int main(int argc, char *argv[]){
  ...
  phase_surr = (float *) malloc(dim * sizeof(float));    
  ...
  while (nodo < numnodos){
	...
	data1[i] = aux + 0.*I;
      ...
      for (i=0;i<dim;i++){
	phase_surr[i] = atan2(creal(hlbrt1[i]),creal(data1[i]));
      }

      // computes phase correlation between node "nodo" and surrogate "cnt-1" for different delays -dim/2 < delay < dim/2
	...
	for (j=0;j<dim-delay;j++){
	  correlsurrtime[delay+index] += cos(phase_node[nodo][j+delay]-phase_surr[j])+I*sin(phase_node[nodo][j+delay]-phase_surr[j]);
	}
	...
	correlsurrtime[delay+index] = sqrt(creal(correlsurrtime[delay+index])*creal(correlsurrtime[delay+index])+cimag(correlsurrtime[delay+index])*cimag(correlsurrtime[delay+index]));
      ...
      correlsurr[nodo][cnt-1] = correlsurrtime[0];
      printf("cnt = %i\n",cnt-1);
      for (delay=1;delay<=dim;delay++){
	if (correlsurrtime[delay] > correlsurr[nodo][cnt-1])    correlsurr[nodo][cnt-1] = correlsurrtime[delay];
      }
    }
    
    // Compute hist 
    for (i=0;i<nbin;i++) hist[i] = 0.;
    maxx = correlsurr[nodo][0];
    minn = correlsurr[nodo][0];
    for (i=1;i<numsur;i++){
      if (correlsurr[nodo][i] > maxx)   maxx = correlsurr[nodo][i];
      if (correlsurr[nodo][i] < minn)   minn = correlsurr[nodo][i];
    }
    dx = (float)((maxx-minn)/nbin);
    for (i=0;i<numsur;i++){
      j = floor((correlsurr[nodo][i]-minn)/dx);
    ...
    if (prob>0.95){
      prob -= hist[i];
      // linear regression:
      threshold[nodo] = (i-1)*dx+(0.95-prob)/(hist[i+1]-hist[i])*dx;
    }
    else threshold[nodo] = i*dx;
  ...
  // filter correlnodes according to thresholds
  for (i=0;i<numnodos-1;i++){
    for (k=i+1;k<numnodos;k++){
      aux = threshold[i];
      if (threshold[k] > aux) aux = threshold[k];
      if (correlnodes[i][k] < aux){
	correlnodes[i][k] = 0.;
	correlnodes[k][i] = 0.;
      }
    }
  ...
  fp = fopen("functional_matrix_old.dat","w");
  for (i=0;i<numnodos;i++){
